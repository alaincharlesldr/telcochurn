...
class DataAnalyzer:
    ...
    def __init__(self, data: Union[str, pd.DataFrame]) -> None:
        ...
        Args:
            data (Union[str, pd.DataFrame]): Either a path to the dataset on HuggingFace
                or a pandas DataFrame containing the customer data.
        """
        if isinstance(data, str):
            self.telco = load_huggingface_dataset(data)
        elif isinstance(data, pd.DataFrame):
            self.telco = data
        else:
            raise TypeError("data must be either a string (dataset path) or a pandas DataFrame")
